...
namespace Oculus.Interaction.GrabAPI
{
    ...
    /// </summary>
    public class PinchGrabConfigurableAPI : MonoBehaviour, IFingerAPI
    {
        [Serializable]
        public struct PinchThresholds
        {
            [Tooltip("The distance threshold at which a finger tip is considered to start pinching.")]
            public float distanceStart;
            [Tooltip("The maximum distance at which a finger tip pinch is considered to stop.")]
            public float distanceStopMax;
            [Tooltip("The distance offset from the minimum pinch distance at which a finger tip pinch is considered to stop.")]
            public float distanceStopOffset;
            [Tooltip("Consider the System Pinch in addition to the thresholds to decide if the finger is pinching.")]
            public bool useSystemPinch;

            public static PinchThresholds DefaultFinger => new PinchThresholds()
            {
                distanceStart = 0.02f,
                distanceStopMax = 0.03f,
                distanceStopOffset = 0.02f,
                useSystemPinch = true,
            };

            public static PinchThresholds DefaultThumb => new PinchThresholds()
            {
                distanceStart = 0.03f,
                distanceStopMax = 0.05f,
                distanceStopOffset = 0.04f,
                useSystemPinch = false,
            };
        }

        [field: SerializeField]
        [Tooltip("The thresholds to detect if the index is pinching.")]
        /// <summary>
        /// The thresholds to detect if the index is pinching.
        /// </summary>
        public PinchThresholds IndexThresholds { get; set; } = PinchThresholds.DefaultFinger;

        [field: SerializeField]
        [Tooltip("The thresholds to detect if the middle is pinching.")]
        /// <summary>
        /// The thresholds to detect if the middle is pinching.
        /// </summary>
        public PinchThresholds MiddleThresholds { get; set; } = PinchThresholds.DefaultFinger;

        [field: SerializeField]
        [Tooltip("The thresholds to detect if the ring is pinching.")]
        /// <summary>
        /// The thresholds to detect if the ring is pinching.
        /// </summary>
        public PinchThresholds RingThresholds { get; set; } = PinchThresholds.DefaultFinger;

        [field: SerializeField]
        [Tooltip("The thresholds to detect if the pinky is pinching.")]
        /// <summary>
        /// The thresholds to detect if the pinky is pinching.
        /// </summary>
        public PinchThresholds PinkyThresholds { get; set; } = PinchThresholds.DefaultFinger;

        [field: SerializeField]
        [Tooltip("The thresholds to detect if the thumb is pinching.")]
        /// <summary>
        /// The thresholds to detect if the thumb is pinching.
        /// </summary>
        public PinchThresholds ThumbThresholds { get; set; } = PinchThresholds.DefaultThumb;

        [field: SerializeField]
        [Tooltip("The maximum dot product value allowed between the thumb and index directions for pinch detection.")]
        /// <summary>
        /// The maximum dot product value allowed between the thumb and finger directions for pinch detection.
        /// </summary>
        public float ThumbMaxDot { get; set; } = 0.5f;

        private readonly HandJointId[] _thumbJointsSelect = new[]
        {
            HandJointId.HandThumb3,
            HandJointId.HandThumbTip
        };

        private readonly HandJointId[] _thumbJointsMaintain = new[]
        {
        ...
        private readonly HandJointId[] _indexJoints = new[]
        {
        ...
        private class FingerPinchData
        {
            ...
            public void UpdateIsPinching(float distance, float start, float stopOffset, float stopMax, bool systemPinch)
            {
                if (!IsPinching)
                {
                    if (systemPinch || distance < start)
                    {
                        IsPinching = true;
        ...
        bool IFingerAPI.GetFingerIsGrabbingChanged(HandFinger finger, bool targetPinchState)
        {
        ...
        {
            hand.GetJointPosesLocal(out var handPoses);
            ...
            UpdateThumb(hand, ThumbThresholds);
            UpdateFinger(hand, HandFinger.Index, IndexThresholds);
            UpdateFinger(hand, HandFinger.Middle, MiddleThresholds);
            UpdateFinger(hand, HandFinger.Ring, RingThresholds);
            UpdateFinger(hand, HandFinger.Pinky, PinkyThresholds);
        }

        private void UpdateThumb(IHand hand, PinchThresholds thresholds)
        {
            int fingerIndex = (int)HandFinger.Thumb;
            _fingersPinchData[fingerIndex].UpdateTipPosition(_shadowHand);

            float distance = float.PositiveInfinity;
            if (IsThumbNearIndex(hand.Handedness))
            {
                ...
                distance = GetClosestDistanceToJoints(thumbStart, thumbEnd, _indexJoints, ThumbMaxDot);
            }

            UpdatePinchData(distance, fingerIndex,
                thresholds.distanceStart, thresholds.distanceStopOffset, thresholds.distanceStopOffset,
                thresholds.useSystemPinch && hand.GetFingerIsPinching(HandFinger.Thumb));
        }

        private void UpdateFinger(IHand hand, HandFinger finger, PinchThresholds thresholds)
        {
            ...
            {
                distance = GetClosestDistanceToJoints(_fingersPinchData[fingerIndex].TipPosition, _thumbJointsMaintain);
            }
            if (IsPointNearThumb(_fingersPinchData[fingerIndex].TipPosition, _thumbJointsSelect))
            {
                distance = GetClosestDistanceToJoints(_fingersPinchData[fingerIndex].TipPosition, _thumbJointsSelect);
            }

            UpdatePinchData(distance, fingerIndex,
                thresholds.distanceStart, thresholds.distanceStopOffset, thresholds.distanceStopMax,
                thresholds.useSystemPinch && hand.GetFingerIsPinching(finger));
        }
        ...
        private bool IsThumbNearIndex(Handedness handedness)
        {
            ...
            return distanceToPlane > 0 && distanceToPlane < ThumbThresholds.distanceStopMax;
        }
        ...
        private void UpdatePinchData(float distance, int fingerIndex,
            float distanceStart, float distanceStopOffset, float distanceStopMax, bool forcePinch)
        {
            _fingersPinchData[fingerIndex].UpdateIsPinching(distance,
                distanceStart, distanceStopOffset, distanceStopMax, forcePinch);
